...
系统 SHALL 处理具有相同基础名称但不同罗马数字后缀的板块。

#### Scenario: 变体值相同或相近时合并
- **WHEN** Tushare返回如"IT服务Ⅱ"和"IT服务Ⅲ"，且两者值相同或差异小于最大值的1%
- **THEN** 合并为基础名称"IT服务"，使用单一值（任一变体的值）
- **AND** 合并后的数据用于排序和连接

#### Scenario: 变体值差异显著时保留两者
- **WHEN** Tushare返回如"其他电源设备Ⅱ"和"其他电源设备Ⅲ"，且两者值差异大于1%
- **THEN** 保留两个条目，各自使用原始名称（含罗马数字后缀）
- **AND** 两者分别排序和连接
